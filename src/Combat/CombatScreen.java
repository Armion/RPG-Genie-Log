	...
	public void keyPressed(int key, char c)
	{
		...
		if(key==Input.KEY_ENTER)
		{
			...
			else if(this.status==12 || this.status==13)
			{
				...
				Team.getInstance().getInventory().useItem(this.objets.get(this.objet).getId(),this.combat.ciblage(current, true).get(curseur));
				}
				else {
				
				Team.getInstance().getInventory().useItem(this.objets.get(this.objet).getId(),this.combat.ciblage(current, false).get(curseur));
				}
